...
class PokemonListViewModel @Inject constructor(
    ...
    fun getPokemonTypes(number: Int): List<String> {
        val pokemon = cachedPokemonList.find { it.number == number }
        return pokemon?.type ?: listOf("Unknown")
    }
            ...
            when (result) {
                ...
                is Resource.Success -> {
                    val pokedexEntries = result.data!!.results.map { data ->
                        ...
                        // Llama a getPokemonTypes para obtener los tipos
                        val types = getPokemonTypes(number.toInt())

                        PokedexListEntry(
                            number = number.toInt(),
                            name = data.name.replaceFirstChar { it.titlecase(Locale.ROOT) },
                            imageUrl = url,
                            type = types
                        )
                    }
